-- Bug found by tests/rls/complete-sale.test.ts: the per-line stock pre-check referenced a bare
-- `quantity` column, which doesn't exist on public.products (it exists on sale_line_items, via
-- the loop variable v_line). This silently failed every checkout with "column quantity does not
-- exist" -- fixed by qualifying it as v_line.quantity, matching the RPC's own doc comment.
...
begin
  ...
  loop
    select current_stock, v_line.quantity * sale_to_stock_factor
